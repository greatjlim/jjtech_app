...
class ProcessLog(Document):
	...
	def validate_sequence(self):
		if self.process_type == STAGE_ORDER[0]:
			self.validate_extrusion_completed()
			return
	...
	def validate_extrusion_completed(self):
		completed = frappe.db.exists("Extrusion Job", {"work_order": self.work_order, "status": "완료"})
		if not completed:
			frappe.throw(_("{0}을(를) 시작하려면 먼저 압출작업이 완료되어야 합니다.").format(self.process_type))
