...
class ProcessLog(Document):
	...
	def validate_sequence(self):
		if self.process_type == STAGE_ORDER[0]:
			self.validate_preheat_checked_out()
			return
	...
	def validate_preheat_checked_out(self):
		checked_out = frappe.db.exists("Mold Preheat Job", {"work_order": self.work_order, "zone": "0"})
		if not checked_out:
			frappe.throw(_("{0}을(를) 시작하려면 먼저 금형이 대출(예열로에서 인출)되어야 합니다.").format(self.process_type))
