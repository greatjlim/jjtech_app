...
import {
  cancelPurchaseOrder,
  deletePurchaseOrder,
  getPurchaseOrderFieldOptions,
  listPurchaseOrderItems,
  listPurchaseOrders,
  submitPurchaseOrder,
  STATUS_LABELS,
  type PurchaseOrderItemListItem,
  type PurchaseOrderListItem,
} from '@/api/purchaseOrder'
import { ApiError } from '@/api/client'
import PurchaseOrderRowActions from '@/components/purchaseOrder/PurchaseOrderRowActions.vue'
import PurchaseOrderModifyPopup from '@/components/purchaseOrder/PurchaseOrderModifyPopup.vue'
import PurchaseOrderRegisterPopup from '@/components/purchaseOrder/PurchaseOrderRegisterPopup.vue'
...
onMounted(async () => {
  statusOptions.value = await getPurchaseOrderFieldOptions('status')
})
...
const submitOrder = async (name: string) => {
  if (!window.confirm('이 발주를 제출하시겠습니까? 제출 후에는 수정할 수 없습니다.')) return
  try {
...
const cancelOrder = async (name: string) => {
  ...
  try {
    await cancelPurchaseOrder(name)
...
const deleteOrder = async (name: string) => {
  if (!window.confirm('발주 정보를 삭제하시겠습니까?')) return
  try {
    await deletePurchaseOrder(name)
...
const orderColumnDefs: ColDef<PurchaseOrderListItem>[] = [
  { headerName: '발주번호', field: 'name', width: 160 },
  { headerName: '발주일자', field: 'transaction_date', width: 120 },
  {
    headerName: '진행상태',
    field: 'status',
    width: 110,
  ...
  { headerName: '납기예정일', field: 'schedule_date', width: 130 },
  { headerName: '비고', field: 'custom_remark', flex: 1 },
  {
    headerName: '실행',
    sortable: false,
    cellRenderer: PurchaseOrderRowActions,
...
const onOrderSelectionChanged = (event: SelectionChangedEvent<PurchaseOrderListItem>) => {
  const rows = event.api.getSelectedRows()
  selectedOrderName.value = rows[0]?.name ?? ''
}

const onOrderCellClicked = (event: CellClickedEvent<PurchaseOrderListItem>) => {
  if (!event.colDef.field || !event.data) return
  editOrder(event.data.name)
}

let debounceHandle: number | undefined
watch([status, searchQuery, orderDateStart, orderDateEnd], () => {
  window.clearTimeout(debounceHandle)
  debounceHandle = window.setTimeout(refreshOrderGrid, 400)
})

// 발주상세내역 그리드 (읽기전용, 선택된 발주의 라인)
...
const lineColumnDefs: ColDef<PurchaseOrderItemListItem>[] = [
...
const buildLineDatasource = (): IDatasource => ({
  ...
  getRows: async (params: IGetRowsParams) => {
    try {
      const { items, total } = await listPurchaseOrderItems(selectedOrderName.value, params.startRow, params.endRow - params.startRow)
...
<template>
  <h1 class="text-h5 font-weight-bold mb-4">발주관리</h1>
  <v-card elevation="2">
    <v-card-text>
      ...
      <SearchDetailPanel v-model="showAdvanced">
        <v-row>
          <v-col cols="12" md="6" lg="3">
            <v-text-field v-model="orderDateStart" type="date" label="발주일자(시작)" variant="outlined" density="comfortable" />
          </v-col>
          <v-col cols="12" md="6" lg="3">
            <v-text-field v-model="orderDateEnd" type="date" label="발주일자(종료)" variant="outlined" density="comfortable" />
          </v-col>
        </v-row>
      </SearchDetailPanel>
  ...
  <PurchaseOrderModifyPopup v-model="showModify" :name="editingName" @saved="onSaved" @error="notifyError" />
  <PurchaseOrderRegisterPopup v-model="showRegister" @saved="onSaved" @error="notifyError" />
